...
var columnDefs = [
    ...
    { headerName: $("#hdn_Username").val(), field: "UserName", tooltip: function (params) { return (params.value); }, width: 80, filter: false, hide: IsObj },
    { headerName: $("#hdn_CustomerName").val(), field: "cust_name", tooltip: function (params) { return (params.value); }, width: 100, filter: false, hide: IsObj },
    {
    ...
    { headerName: $("#hdn_Rap_Price_Doller").val(), field: "cur_rap_rate", tooltip: function (params) { return formatNumber(params.value); }, width: 80, cellRenderer: function (params) { if (params.value != "0") { return formatNumber(params.value); } }, filter: false },
    { headerName: $("#hdn_Rap_Amt_Doller").val(), field: "rap_amount", width: 90, tooltip: function (params) { return formatNumber(params.value); }, cellRenderer: function (params) { if (params.value != "0") { return formatNumber(params.value); } }, filter: false },

    { headerName: $("#hdn_Offer_Disc_Per").val(), field: "sales_disc_per", width: 90, tooltip: function (params) { return parseFloat(params.value).toFixed(2); }, cellStyle: { color: 'red', 'font-weight': 'bold' }, cellRenderer: function (params) { if (params.value != "0") { return parseFloat(params.value).toFixed(2); } }, filter: false },
    ...
    { headerName: $("#hdn_girdle").val(), field: "girdle_per", tooltip: function (params) { return formatNumber(params.value); }, width: 60 },
...
function SendMail() {
    ...
    var stoneno = '';
    var count = 0;
    var stoneno_, iUserid_, iUserid_stoneno_;
    ...
    if ($('#customRadiomail').prop('checked')) {
        ...
        var stoneno = _.pluck(gridOptions.api.getSelectedRows(), 'stone_ref_no').join(",");
        var count = gridOptions.api.getSelectedRows().length;
        stoneno_ = _.pluck(gridOptions.api.getSelectedRows(), 'stone_ref_no');
        iUserid_ = _.pluck(gridOptions.api.getSelectedRows(), 'iUserid');
        iUserid_stoneno_ = ([iUserid_, stoneno_].reduce((a, b) => a.map((v, i) => v + b[i]))).join(",");
    }
    var _data = {
        IsAdmin: 'false',
        RefNo: stoneno,
        PageNo: '0',
        FormName: 'My Wishlist',
        ActivityType: 'Excel Export',
        iUserid_certi_no: iUserid_stoneno_,
        SubUser: SubUser,
        IsAssistBy: ($("#hdnIsPrimary").val() == "True" ? true : false),
        ToAddress: $('#txtemail').val(),
    ...
    if (count > 0) {
        $.ajax({
            url: "/SearchStock/MyWishlist_EmailSelectedStone",
            type: "POST",
            data: _data,
            success: function (data, textStatus, jqXHR) {
                if (data.Status == "0") {
...
function DownloadExcel() {
    ...
    if (count > 0) {
        ...
        obj1.iUserid_certi_no = iUserid_stoneno_;
        obj1.SubUser = SubUser;
        obj1.IsAssistBy = ($("#hdnIsPrimary").val() == "True" ? true : false)
        $.ajax({
